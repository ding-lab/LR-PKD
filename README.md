...
* git
* conda
* GATK (optional): As recommended, running Long Ranger with GATK for more accurate calling of SNPs and indels. Users need to have a working version of GATK installed on the working system before running LR-PKD. Long Ranger uses GATK's HaplotypeCaller mode. See the GATK installation instructions (https://software.broadinstitute.org/gatk/documentation/quickstart). Support is also provided for using a bundled build of Freebayes which will be installed within Long Ranger. If users prefer to use Freebayes, please skip the step of installing GATK and define in config.sh.
