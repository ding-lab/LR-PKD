...
file1 = pd.read_table('input_d1.vep.maf', na_filter=False, float_precision=None)
file2 = pd.read_table('input.vep.vcf_GT_PS_BX.txt', na_filter=False, float_precision=None)
merge_df = pd.merge(file1, file2, on=['Chromosome', 'Start_Position'], how='left').fillna(0)
merge_df.to_csv('input.vep.GTPSBX.maf', sep='\t')
